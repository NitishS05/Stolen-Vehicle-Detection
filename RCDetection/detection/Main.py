...
from . import nomralizeRGB, circularMask, timeConvert, arcDetect, contorsHighlight
...
def function(videofile, dist, masknum):

    #User adjustable variables
    videoName = str(videofile)
    frameStart = int(dist)
    circularMaskRadius = int(masknum)
    ...
    worksheet.write('H1', 'Object X location')
        ...
        #If detection is not a circle and apply detection is ON, measure the detection size
            ...
            for contour in contouredImage:
                ...
                if new_blockage_area > nominated_blockage_area:
                    ...
                    cv2.imwrite(str(videodirname+"\\"+str(blockageCounter-1)+".jpg"),frame)
                    continue
